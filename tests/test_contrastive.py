...
def test_disagreements_are_reachable_and_off_terminal():
    ...
    ma, ca = load_agent("checkpoints/navigator.pt")
    mb, cb = load_agent("checkpoints/collector.pt")
...
def test_no_vacuous_same_channel_explanation():
    """When two agents share the dominant channel, the contrast must describe a
    route difference, never the vacuous 'A driven by X while B driven by X'."""
    ma, ca = load_agent("checkpoints/navigator.pt")
    mb, cb = load_agent("checkpoints/balanced.pt")
    dis = find_trajectory_disagreements(ma, ca["weights"], mb, cb["weights"])
    for d in dis:
        t = generate_explanation(d, "A", "B")
        for ch in ("goal", "safety", "coin", "step"):
            assert f"driven by {ch}, while B is driven by {ch}" not in t
